...
import { Zap, Mail, Lock, ArrowRight, User, ChevronLeft } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Create profile row with 1 free credit
    if (data.user) {
      ...
      <div className="flex flex-col items-center justify-center p-6 font-sans relative">
        ...
        <div className="max-w-[400px] w-full relative z-10 pt-12">
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-[32px] p-8 backdrop-blur-xl shadow-2xl">
            <h1 className="text-3xl font-bold tracking-tight mb-2 text-white">Comece sua jornada.</h1>
            <p className="text-white/50 text-sm font-medium mb-8">Ganhe 1 crédito grátis para criar sua primeira loja.</p>

            <form onSubmit={handleRegister} className="space-y-4">
              {/* Name Field */}
              <div>
                <label className="block text-[13px] font-bold text-white/40 uppercase tracking-widest mb-1.5 ml-1">Nome completo</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full h-14 bg-white/5 border border-white/10 rounded-2xl px-12 text-[15px] font-medium focus:outline-none focus:border-[#ff0050] transition-colors placeholder:text-white/10 [color-scheme:dark] text-white"
                    required
                  />
                </div>
              </div>
              ...
              {/* Email Field */}
              <div>
                <label className="block text-[13px] font-bold text-white/40 uppercase tracking-widest mb-1.5 ml-1">E-mail</label>
                <div className="relative">
              ...
              {/* Password Field */}
              <div>
                <label className="block text-[13px] font-bold text-white/40 uppercase tracking-widest mb-1.5 ml-1">Senha</label>
                <div className="relative">
            ...
            <div className="mt-8 pt-8 border-t border-white/5">
              <p className="text-center text-sm text-white/40 font-medium">
                Já tem uma conta? <Link href="/login" className="text-white hover:text-[#ff0050] transition-colors">Entre por aqui.</Link>
              </p>
            </div>
          </div>
        </div>
